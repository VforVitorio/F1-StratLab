...
def _print_header(gp: str, driver: str, team: str, total_laps: int) -> None:
    ...
    print(f"\n{_BOLD}{sep}{_RESET}")
    print(
        f"  {_BOLD}{_CYAN}F1 Strategy - Race Replay{_RESET}   "
        f"{_BOLD}{gp}{_RESET}  |  {driver} / {team}  |  {total_laps} laps"
    )
    print(sep)
    print(
        f"  {'Lap':>4}  {'Pos':>3}  {'Compound':>12}  {'LapTime':>8}  "
...
def main() -> None:
    ...
    if not race_dir.exists():
        ...
        print(
            f"Available GPs: {', '.join(sorted(p.name for p in Path(args.data_dir).iterdir() if p.is_dir()))}"
        )
        sys.exit(1)
