...
The RAG retrieval portion of E-11 is now wired into the shared evaluator as
`uv run f1-eval rag`. It uses the 30-query set in
`data/rag_eval/queries_v2.json` and writes
`documents/eval_reports/rag.{md,json}`. The remaining R-9 work is agent-level
citation faithfulness, which needs the actual LangGraph tool trace.

The findings register below is the historical audit snapshot. E-11's retrieval
portion is updated above; the remaining orchestrator and agent findings are not
...
| `N30B_rag_benchmark.ipynb` plus `f1-eval rag` | Historical 15-query notebook benchmark plus the shared 30-query production retrieval evaluation | The notebook remains a historical three-configuration comparison; agent-level citation faithfulness is still outside retrieval evaluation |
...
| **E-11** | **P2** | **Resolved for retrieval by #321.** `f1-eval rag` evaluates 30 manually verified queries, including production question shapes, with P@k, MRR, wrong-year rate, retrieval-level citation match, and latency. Agent-level citation faithfulness remains separate work. | The Qatar citation path still needs the actual LangGraph tool-trace check described by R-9. | **S-M** |
