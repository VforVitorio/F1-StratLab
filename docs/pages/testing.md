...
Issue #1252 has a local trace for one measured Lusail 2025 tick. It captures
one message from the live Arcade TCP server and the real `/api/tick`,
`/api/bulk`, `/api/live`, and `/api/agents` responses. The JSON report records
the complete wire message and each sequence. Check the rendered DATA and
AGENTS pages separately in a browser.
...
`--no-llm` and the empty API key prevent paid model calls. It is a local data
run, not part of the PR gate.
